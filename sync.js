...
const date = (minus) => {
  ...
  return `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`;
    ...
    db.serialize(() => {
        db.run("CREATE TABLE IF NOT EXISTS video_snapshots (id TEXT PRIMARY KEY, date DATE, snapshot TEXT)");
        db.run("CREATE TABLE IF NOT EXISTS channel_snapshots (id TEXT PRIMARY KEY, date DATE, snapshot TEXT)");
        ...
        for (const channelName in config.channels) {
            ...
            db.run("INSERT INTO channel_snapshots(id, date, snapshot) VALUES(?, ?, ?) ON CONFLICT(ID) DO UPDATE SET snapshot = excluded.snapshot", 
                [`${date()}-${channel.id}`, 
                new Date(), 
    ...
    for (const record in videos) {
        ...
        db.run("INSERT INTO video_snapshots(id, date, snapshot) VALUES(?, ?,?) ON CONFLICT(id) DO UPDATE SET snapshot = excluded.snapshot", [`${date()}-${obj.id}`, new Date(), JSON.stringify(obj)]);
    }
    });
    db.close();
